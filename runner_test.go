...
func Test_add01(t *testing.T) {
	...
	total, _, last := r.RunRepack(ts, s, &MyPack_t{id: "1"})
...
func Test_add02(t *testing.T) {
	...
	total, _, last := r.RunRepack(ts, s, &MyPack_t{id: "1"}, &MyPack_t{id: "2"})
...
func Test_add03(t *testing.T) {
	...
	total, _, last := r.RunRepack(ts, s, &MyPack_t{id: "1"}, &MyPack_t{id: "2"}, &MyPack_t{id: "3"})
	assert.Equal(t, total, 2)
	assert.Equal(t, last, 2)
	assert.Equal(t, r.SizeFilter(ts), 2)
	assert.Equal(t, r.SizeQueue(), 2)
...
func Test_add04(t *testing.T) {
	...
	total, _, last = r.RunRepack(ts, s, &MyPack_t{id: "2"}, &MyPack_t{id: "3"})
	assert.Equal(t, total, 1)
	assert.Equal(t, last, 1)
	assert.Equal(t, r.SizeFilter(ts), 2)
	assert.Equal(t, r.SizeQueue(), 2)
...
func Test_add10(t *testing.T) {
	...
	total, _, last := r.RunRepack(ts, s, &MyPack_t{id: "1"}, &MyPack_t{id: "1"}, &MyPack_t{id: "2"})
	assert.Equal(t, total, 2)
