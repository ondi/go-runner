...
func Test_add01(t *testing.T) {
	...
	_, queued, _ := r.RunAny(ts, name, DoSome, DoneSome, 1, &MyPack_t{In: []string{"1"}})
...
func Test_add02(t *testing.T) {
	...
	_, queued, _ := r.RunAny(ts, name, DoSome, DoneSome, 1, &MyPack_t{In: []string{"1", "2"}})
...
func Test_add03(t *testing.T) {
	...
	_, queued, _ := r.RunAny(ts, name, DoSome, DoneSome, 1, &MyPack_t{In: []string{"1", "2", "3"}})
	assert.Equal(t, queued, 2)
	assert.Equal(t, r.SizeFilter(ts), 2)
	assert.Equal(t, r.SizeQueue(), 2)
}
...
func Test_add04(t *testing.T) {
	...
	_, queued, _ = r.RunAny(ts, name, DoSome, DoneSome, 1, &MyPack_t{In: []string{"2", "3"}})
	assert.Equal(t, queued, 1)
	assert.Equal(t, r.SizeFilter(ts), 2)
	assert.Equal(t, r.SizeQueue(), 2)
...
func Test_add10(t *testing.T) {
	...
	_, queued, _ := r.RunAny(ts, name, DoSome, DoneSome, 1, &MyPack_t{In: []string{"1", "1", "2"}})
	assert.Equal(t, queued, 1)
}
